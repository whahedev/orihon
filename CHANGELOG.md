...
- Added declarative MVT paint rules, Standard WMTS REST templates/capabilities parsing, a live local WMTS lab endpoint, and optional zero-dependency `orihon/pmtiles` v3 range reading backed by a committed binary fixture.
...
- Added browser acceptance coverage for mixed renderer queries, repeated 12-point spiderfy, RTL label collision/alignment and live WMTS tile loading.
...
- Advanced gzip budget is 74 KiB without removing Standard/P1 functionality; Core and Standard load map export only on first use, while optional P2 controls and geo helpers keep their own budgets.
...
- Popup/tooltip activation now uses pointer-tap semantics across SVG, canvas, WebGL, markers and media overlays. Chrome no longer loses geometry clicks to map pointer capture; binding an overlay enables interaction automatically, and closed paths remain hittable across their full interior without changing their paint.
- Studio popup video blocks expose autoplay and default to muted inline playback so Chrome can start them when a popup opens; standalone HTML exports preserve the same behavior.
- Studio object placement now owns its pointer gesture, preventing existing markers or closed geometries underneath the cursor from opening their popup while a new object is placed.
- Added optional `orihon/popup-content`: a reusable declarative renderer for safe HTML, text, image, video and adapter-driven charts. Studio and generated code now call this engine helper instead of embedding their own renderer.
