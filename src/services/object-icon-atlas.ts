...
/** Optional id→slot map helpers for swap-remove GPU buffers. */
export class ObjectSlotMap {
  readonly idToSlot = new Map<ObjectId, number>();
  readonly slotToId: ObjectId[] = [];

  get size(): number {
    return this.slotToId.length;
  }
  ...
  slotOf(id: ObjectId): number | undefined {
    return this.idToSlot.get(id);
  }

  idOf(slot: number): ObjectId | undefined {
    return this.slotToId[slot];
  }

  add(id: ObjectId): number {
  ...
  /** Swap-remove. Returns moved id that took the removed slot (if any). */
  remove(id: ObjectId): { removedSlot: number; movedId: ObjectId | null } | null {
    const slot = this.idToSlot.get(id);
    if (slot == null) return null;
    const last = this.slotToId.length - 1;
    const lastId = this.slotToId[last];
    this.slotToId[slot] = lastId;
    this.slotToId.pop();
    this.idToSlot.set(lastId, slot);
    this.idToSlot.delete(id);
    return { removedSlot: slot, movedId: slot === last ? null : lastId };
  }
