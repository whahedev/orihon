...
test("WebGL setObjectState patches color without full setData rebuild", () => {
  const manager = objectManager({
    clusterize: false,
  ...
  manager.addTo(map);
  const layer = [...map.layers].find((entry) => entry.patchColor && entry.setData);
  assert.ok(layer);

  let setDataCalls = 0;
  let setColorsCalls = 0;
  let setSizesCalls = 0;
  let patchColorCalls = 0;
  let patchSizeCalls = 0;
  let renderCalls = 0;
  const setData = layer.setData.bind(layer);
  const setColors = layer.setColors.bind(layer);
  const setSizes = layer.setSizes.bind(layer);
  const patchColor = layer.patchColor.bind(layer);
  const patchSize = layer.patchSize.bind(layer);
  ...
  layer.patchColor = (...args) => {
    patchColorCalls++;
    return patchColor(...args);
  };
  layer.patchSize = (...args) => {
    patchSizeCalls++;
    return patchSize(...args);
  };
  layer.render = (...args) => {
    renderCalls++;
    return render(...args);
  ...
  assert.equal(patchColorCalls, 1);
  assert.equal(patchSizeCalls, 1);
